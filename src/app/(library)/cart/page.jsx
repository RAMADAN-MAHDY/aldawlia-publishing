...
import { Trash2, ShoppingBasket, ArrowRight, ArrowLeft, CreditCard, Lock, Smartphone, CheckCircle } from "lucide-react";
...
const CartPage = () => {
  ...
  const handleStartPayment = async () => {
    ...
    if (!firstBookId) {
      return toast.error(t("cart.book_data_incomplete"));
    }
    ...
    try {
      ...
      if (paymentProvider === 'paymob' && data.data.paymentLink) {
        ...
        if (paymentMethod === 'wallet' && data.data.paymentMethodUsed === 'card') {
          toast.error(t("cart.wallet_payment_failed"));
          return;
        }
      ...
      toast.error(err.response?.data?.message || err.message || t("cart.payment_start_failed"));
    } finally {
      setProcessing(false);
    }
  };
  ...
  if (!isAuthenticated) return (
    <div dir={dir} className="flex flex-col items-center justify-center min-h-[70vh] p-6 text-center">
      ...
      <h2 className="text-xl font-bold text-gray-600">{t("cart.login_to_continue")}</h2>
      <button onClick={() => router.push("/login")} className="bg-amber-600 text-white px-10 py-3 rounded-3xl font-bold mt-4">{t("cart.login")}</button>
    </div>
  );
    ...
    <div className={`bg-[#f8f8f8] min-h-screen pb-32 relative ${isArabic ? "text-right" : "text-left"}`} dir={dir}>
      <div className="bg-white/90 backdrop-blur-md sticky top-0 z-40 p-4 shadow-sm border-b border-gray-100">
        <div className="max-w-7xl mx-auto flex items-center justify-center relative">
          <button onClick={() => router.back()} className={`text-sky-900 absolute p-4 ${isArabic ? "right-0" : "left-0"}`}>{isArabic ? <ArrowRight size={28} /> : <ArrowLeft size={28} />}</button>
          <h1 className="text-sky-900 font-extrabold text-xl">{t("cart.title")}</h1>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4">
        {loading ? (
          <div className="text-center py-20">
            <p className="text-sky-900 font-bold animate-pulse">{t("cart.loading")}</p>
          </div>
          ...
          <>
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        <p className="text-amber-600 font-black text-sm">
                          {(item.file?.isOnSale && item.file?.discountPrice) ? item.file.discountPrice : item.file?.price} {t("cart.currency")}
                        </p>
                        {item.file?.isOnSale && (
                          <span className="text-gray-400 line-through text-[10px]">
                            {item.file?.price?.toLocaleString()} {t("cart.currency")}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            <div className="mt-6 bg-white rounded-[2.5rem] p-6 shadow-xl border border-sky-50 text-center">
              <p className="text-gray-400 font-bold mb-1">{t("cart.total_amount")}</p>
              <h2 className="text-3xl font-black text-sky-900 mb-6">{totalPrice.toLocaleString()} {t("cart.currency")}</h2>
              
              <button
                onClick={() => setPaymentModal(true)}
                className="w-full bg-sky-900 text-white py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-2"
              >
                <CreditCard size={24} /> {t("cart.complete_purchase")}
              </button>
            </div>
          </>
        )}
          ...
          <div className="relative bg-white w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-[2rem] shadow-2xl p-6">
            ...
            {clientSecret && paymentProvider === 'stripe' ? (
              <Elements stripe={stripePromise} options={{ clientSecret }}>
                <div className="flex flex-col">
                  <h2 className="text-lg font-bold text-gray-950 mb-4 text-center">{t("cart.card_details")}</h2>
                  <CheckoutForm clientSecret={clientSecret} redirectionUrl={redirectionUrl} />
                  <button onClick={() => { setClientSecret(""); setRedirectionUrl(""); }} className="mt-6 text-gray-400 text-xs font-bold">{t("cart.back")}</button>
                </div>
              </Elements>
            ) : (
              <div className="flex flex-col items-center text-center">
                ...
                <h2 className="text-xl font-bold text-gray-950 mb-6">{t("cart.complete_payment")}</h2>
                ...
                <div className="w-full space-y-2 mb-6">
                  ...
                  <button onClick={() => { setPaymentProvider("paymob"); setPaymentMethod("wallet"); }} className={`w-full flex items-center justify-between p-4 rounded-xl border-2 ${paymentProvider === 'paymob' && paymentMethod === 'wallet' ? 'border-sky-900 bg-sky-50' : 'border-gray-100'}`}>
                    <div className="flex items-center gap-3"><Smartphone size={18}/><span className="font-bold text-xs">{t("cart.wallet_option")}</span></div>
                  ...
                  <button onClick={() => { setPaymentProvider("paymob"); setPaymentMethod("card"); }} className={`w-full flex items-center justify-between p-4 rounded-xl border-2 ${paymentProvider === 'paymob' && paymentMethod === 'card' ? 'border-sky-900 bg-sky-50' : 'border-gray-100'}`}>
                    <div className="flex items-center gap-3"><CreditCard size={18}/><span className="font-bold text-xs">{t("cart.paymob_card_option")}</span></div>
                  ...
                  <button onClick={() => { setPaymentProvider("stripe"); setPaymentMethod("card"); }} className={`w-full flex items-center justify-between p-4 rounded-xl border-2 ${paymentProvider === 'stripe' ? 'border-sky-900 bg-sky-50' : 'border-gray-100'}`}>
                    <div className="flex items-center gap-3"><CreditCard size={18}/><span className="font-bold text-xs">{t("cart.stripe_card_option")}</span></div>
                ...
                {paymentProvider === 'paymob' && paymentMethod === 'wallet' && (
                  <input type="tel" placeholder={t("cart.wallet_phone_placeholder")} value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className="w-full bg-gray-50 border border-gray-100 p-3 rounded-xl mb-4 text-center outline-none focus:ring-1 focus:ring-sky-900"/>
                )}

                <button onClick={handleStartPayment} disabled={processing} className="w-full py-4 bg-sky-900 text-white rounded-xl font-bold">
                  {processing ? t("cart.processing") : t("cart.confirm_pay")}
                </button>
                <button onClick={() => setPaymentModal(false)} className="mt-4 text-gray-400 text-xs">{t("cart.cancel")}</button>
              </div>
            )}
          </div>
        </div>
